...
export const pfnMachine = setup({
  types: {
    ...
    events: /** @type {(
      ...
      | { type:'SET_CARDS', cards:{id:string, easy:string, hard:string}[] }
      | { type:'START_GAME' }
      | { type:'START_TURN' }
      | { type:'GUESS_3' }
      | { type:'GUESS_1' }
  ...
  actions: {
    ...
    nextPlayer: ({ context }) => { 
      ...
      context.currentTeamIndex = Math.floor(context.currentPlayerIndex / context.playersPerTeam) % context.teams.length;
    },
